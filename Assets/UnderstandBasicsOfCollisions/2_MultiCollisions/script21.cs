...
public class script21 : MonoBehaviour {
	...
	void Update () {
		switch(objectmove)	{
			// move phase
		case 0: 	GetComponent<Rigidbody>().transform.Translate(-0.05f,0,0);
					break;
			// back and rotate phase when collide
		case 1:		GetComponent<Rigidbody>().transform.Translate(2.5f,0,0);
					GetComponent<Rigidbody>().transform.Rotate(0,22,0, Space.Self);
					objectmove=0;
					break;
		}
	...
	void OnCollisionEnter(Collision collision) {
		...
		if(collision.gameObject.name.Contains("Cube8"))	{
			...
			collision.gameObject.GetComponent<Renderer>().material.color = new Color(1,0.5f,0,1);
		}
		else {
